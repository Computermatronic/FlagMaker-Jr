	...
	for (var i = 0; i < overlay.sliders.length; i++) {
		...
		if(typeof overlayName !== "undefined"){
			...
			string += "<label for=\"slider-" + name + "-" + i + "\">" + label + "</label>" +
			          "<input type=\"range\" name=\"" + name + "\" id=\"" + name + "\" data-highlight=\"true\" min=\"0\" max=\"" + (maxType == "y-axis" ? maxY : (maxType == "x-axis" ? maxX : maxType)) + "\" value=\"" + overlayProperties[overlayName][i + 2] + "\" step=\".1\" ";
		} else {
			overlayProperties["overlay" + id][i + 2] = overlay.sliders[i][2];
			string += "<label for=\"slider-" + name + "-" + i + "\">" + label + "</label>" +
			          "<input type=\"range\" name=\"" + name + "\" id=\"" + name + "\" data-highlight=\"true\" min=\"0\" max=\"" + (maxType == "y-axis" ? maxY : (maxType == "x-axis" ? maxX : maxType)) + "\" value=\"" + overlay.sliders[i][2] + "\" step=\".1\" ";
		}
		if (maxType == "x-axis") {
			string += "use-x ";
